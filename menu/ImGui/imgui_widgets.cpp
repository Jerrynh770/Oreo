...
bool ImGui::beginchildex(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    parent_window->DrawList->AddRectFilled(ImGui::GetWindowPos(), ImGui::GetWindowPos() + ImVec2(size_arg.x, size_arg.y), ImColor(20, 20, 20, (int)(GetStyle().Alpha * 255.f)), 6, ImDrawCornerFlags_Bot);
    parent_window->DrawList->AddRect(ImGui::GetWindowPos() - ImVec2(0, 32), ImGui::GetWindowPos() + size_arg, ImColor(33, 33, 33, int(255 * g.Style.Alpha)), 6);
    parent_window->DrawList->AddRect(ImGui::GetWindowPos() - ImVec2(0, 33), ImGui::GetWindowPos() + size_arg, ImColor(40, 40, 40, int(255 * g.Style.Alpha)), 6);
...
bool ImGui::tab(const char* icon, const char* label, bool selected)
{
    ...
    it_hover->second = math::clamp(it_hover->second + (2.15f * GetIO().DeltaTime * (hovered ? 1.f : -1.f)), 0.0f, 0.0f);
    it_hover->second *= GetStyle().Alpha;
    static std::map<ImGuiID, float> filled_animation;
    auto it_filled = filled_animation.find(id);
    if (it_filled == filled_animation.end())
    ...
    it_filled->second = math::clamp(it_filled->second + (2.15f * GetIO().DeltaTime * (selected ? 1.f : -1.5f)), it_hover->second, 1.f);
    ...
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x - 6, bb.Min.y - 6), ImVec2(bb.Max.x + 6, bb.Max.y + 6), ImColor(40, 40, 40, int(5 * it_filled->second)), 4);
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x - 5, bb.Min.y - 5), ImVec2(bb.Max.x + 5, bb.Max.y + 5), ImColor(40, 40, 40, int(10 * it_filled->second)), 4);
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x - 4, bb.Min.y - 4), ImVec2(bb.Max.x + 4, bb.Max.y + 4), ImColor(40, 40, 40, int(15 * it_filled->second)), 4);
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x - 3, bb.Min.y - 3), ImVec2(bb.Max.x + 3, bb.Max.y + 3), ImColor(40, 40, 40, int(25 * it_filled->second)), 4);
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x - 2, bb.Min.y - 2), ImVec2(bb.Max.x + 2, bb.Max.y + 2), ImColor(40, 40, 40, int(35 * it_filled->second)), 4);
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x - 1, bb.Min.y - 1), ImVec2(bb.Max.x + 1, bb.Max.y + 1), ImColor(40, 40, 40, int(55 * it_filled->second)), 4);
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), ImColor(40, 40, 40, int(225 * it_filled->second)), 4);
    GetWindowDrawList()->AddRect(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), ImColor(60, 60, 60, int(255 * it_filled->second)), 4);
    ...
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + icon_size.x / 2, bb.Min.y + size.y / 2 - icon_size.y / 2) + add, ImColor(100, 100, 100, int(200 * GetStyle().Alpha)), icon);
    if (selected)
        GetWindowDrawList()->AddText(ImVec2(bb.Min.x + icon_size.x / 2, bb.Min.y + size.y / 2 - icon_size.y / 2) + add, ImColor(100, 125, 200, int(255 * it_filled->second)), icon);

    PushFont(c_menu::get().g_pIconsFont_Sub);
    auto label_size = CalcTextSize(label);
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 15, bb.Min.y + size.y / 2 - label_size.y / 2), ImColor(100, 100, 100, int(200 * GetStyle().Alpha)), label);
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 15, bb.Min.y + size.y / 2 - label_size.y / 2), ImColor(100, 125, 200, int(255 * it_filled->second)), label);
    PopFont();

...
void misc::watermark()
{
	...
	if (ImGui::Begin("Watermark", NULL, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar))
	{
		...
		std::vector<std::string> watermark = { "Oreo || Free cheats || ping: " + std::to_string(g_ctx.globals.ping) };
...
void misc::ChatSpammer()
{
	...
	{
		crypt_str("Oreo - good cheat for the game."),
		crypt_str("Get good. Get Oreo."),
		crypt_str("Oreo - better than its competitors."),
		crypt_str("Go to another level with Oreo."),
	};
